package tech.codegarage.dropdownmenuplus;
...
import tech.codegarage.dropdownmenuplus.util.SimpleAnimationListener;
import tech.codegarage.dropdownmenuplus.view.FilterTabsIndicator;
...
public class DropDownMenu extends RelativeLayout implements View.OnClickListener, FilterTabsIndicator.OnItemClickListener {
    ...
    private int mTabBackgroundColorId;
    ...
    protected void initAttrs(AttributeSet attrs) {
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.DropDownMenu);
        try {
            mContentViewId = typedArray.getResourceId(R.styleable.DropDownMenu_layout_content, 0);
            mTabBackgroundColorId = typedArray.getResourceId(R.styleable.DropDownMenu_tab_background, 0);
            mShowTab = typedArray.getBoolean(R.styleable.DropDownMenu_show_tab, false);
        } finally {
            typedArray.recycle();
        }
    }
    ...
    protected void onFinishInflate() {
        ...
        mContainerView = findViewById(R.id.layout_content_container);

        if (mContentViewId != 0) {
            View view = LayoutInflater.from(getContext()).inflate(mContentViewId, mContainerView);
            setContentView(view);
        }
        if (mTabBackgroundColorId != 0) {
            setDropDownTabBackground(mTabBackgroundColorId);
        }
        visibleDropDownTab((mShowTab) ? VISIBLE : GONE);

//        View view = findViewById(R.id.layout_loaddata);
//        setContentView(view);//mFilterRecyclerView layout_load_data
    }

    public void visibleDropDownTab(int isVisibleDropDown) {
        filterTabsIndicator.setVisibility(isVisibleDropDown);
    }

    public void setDropDownTabBackground(int color) {
        filterTabsIndicator.setBackgroundColor(color);
    }

    public void setTabDefaultTextColor(int defaultColor) {
        filterTabsIndicator.setTabDefaultColor(defaultColor);
    }

    public void setTabSelectedTextColor(int selectedColor) {
        filterTabsIndicator.setTabSelectedColor(selectedColor);
    }
