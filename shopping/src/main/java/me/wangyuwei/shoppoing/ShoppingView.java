...
public class ShoppingView extends View {
    ...
    private BitmapDrawable mAddIcon;
    private Canvas mAddCanvas;
    ...
    private void init(AttributeSet attrs) {
        ...
        int numberColor = typeArray.getColor(R.styleable.ShoppingView_sv_number_color, ContextCompat.getColor(getContext(), R.color.sv_white));
        typeArray.recycle();
        ...
        mPaintNum = new Paint();
        mPaintNum.setColor(numberColor);
    ...
    public Bitmap getAddIcon() {
        if (mAddIcon == null) {
            mAddIcon = new BitmapDrawable();
            mAddIcon.draw(mAddCanvas);
        }
        return mAddIcon.getBitmap();
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (mState == STATE_NONE) {
                    Log.d("ShoppingView", "mState == STATE_NONE");
//                    mNum++;
                    startMoveAnim();
