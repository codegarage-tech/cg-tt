 ...
 */
public class PhoneNumberEditTextWithMask extends TextInputEditText implements CountryPickerDialog.OnCountrySelectedCallback {

    private static final int EXTRA_PADDING = 5;
    private static final String TAG = PhoneNumberEditTextWithMask.class.getSimpleName();
    ...
    private boolean isPickerEnable = true;
    ...
    public PhoneNumberEditTextWithMask(Context context) {
        this(context, null);

    }

    public PhoneNumberEditTextWithMask(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public PhoneNumberEditTextWithMask(Context context, AttributeSet attrs, int defStyleAttr) {
...
//                searchAllowed = a.getBoolean(R.styleable.PhoneNumberEditText_cp_searchAllowed, true);
                 ...
                 * Mask *
                 ********/
                mask = (a.getString(R.styleable.PhoneNumberEditText_cp_mask) == null) ? "" : a.getString(R.styleable.PhoneNumberEditText_cp_mask);
                placeholder = String.valueOf(((a.getString(R.styleable.PhoneNumberEditText_cp_placeholder) == null) ? ' ' + "" : a.getString(R.styleable.PhoneNumberEditText_cp_placeholder)));
            } finally {
                a.recycle();
            }
            ...
            if (mask.length() > 0)
                // sets the text to create the mask
                setText(getText());
        }
...
//        }
    ...
    public String getBangladeshNumber() {
        String removeSpecialCharacter = getFullNumberWithPlus().replace("+", "").replace("-", "").trim();
    ...
    private Country getCountryForName(Language languageToApply, String countryCode) {
        ...
        return new Country("Bangladesh", "+880", "BD");
    ...
    public void setPickerEnable(boolean pickerEnable) {
        isPickerEnable = pickerEnable;
    }
    ...
    private void loadLastSelectedCountryCode() {
        ...
        String lastSelectedCode = preferences.getString(LAST_SELECTION_TAG, "bd");
    ...
    public void setOnTextChangeListener(PhoneNumberEditTextWithMask.onTextChangeListener onTextChangeListener) {
